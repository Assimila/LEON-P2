...
## Abundance and similarity models: effects of land use

Oil palm plantations exhibit average declines in abundance exceeding XX% compared to primary vegetation.
...
Predict_BII <- function(UML_data, 
  ...
  for (i in 1:nrow(UML_data)){
    ...
    writeRaster(
      Raster_outout,
      paste0(
        '../results/2_biodiversity_metrics_site_level/BII_rasters_UML/',
        UML_data$UML.ID[i],
        '.tif'
      ),
...
ggplot(Set, aes(Median_BII)) + geom_histogram() + 
  theme_bw() + 
  geom_vline(xintercept = GlobalMedian, col='red', lty='dashed') +
...
Example <- rast("../results/2_biodiversity_metrics_site_level/BII_rasters_UML/PO1000000190.tif")
plot(Example, main='BII - Mill ID: PO1000000052')
```

# 4. CONCLUSIONS AND FURTHER DEVELOPMENTS

We found significant biodiversity declines in oil-palm plantations compared to primary vegetation. Total abundance declined by an average 56%, species richness by an average 48%, and similarity by an average 45%.

However we did not detect any significant effects of oil palm age. Therefore the projections of BII were spatially homogenous in oil palm plantations.

Plantations that were undetected in the EO-data, but recorded in-situ from PREDICTS, did not show significant declines in biodiversity. Such plantations might be smallholder plantations surrounded by extensive primary vegetation.

Current challenges:

-   finding relevant Earth Observation products that can be matched to the in-situ biodiversity data, to help understand biodiversity changes;the biodiversity data was sampled at different time points (some of the records going back to the 1980s in the PREDICTS database).

-   Mapping areas other than oil palm plantations to have a better representation of BII in the landscape.
